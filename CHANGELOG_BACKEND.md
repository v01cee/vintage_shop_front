...
### [2025-12-14] - Реализация фоновой задачи для отмены неоплаченных заказов

**Выполнено:**
- Добавлена фоновая задача для автоматической отмены неоплаченных заказов
- Настроен APScheduler для периодического запуска задачи (каждую минуту)
- Реализована функция `cancel_unpaid_orders()` для отмены заказов с истекшим резервом
- Добавлено логирование операций отмены заказов

**Как работает:**
- Задача запускается автоматически каждую минуту
- Проверяет заказы со статусом "new", у которых `reserved_until` уже прошло
- Автоматически меняет статус таких заказов на "cancelled"
- Логирует все операции отмены для отслеживания

**Файлы:**
- `backend/app/tasks/order_tasks.py` (новый) - функция для отмены неоплаченных заказов
- `backend/app/tasks/__init__.py` (новый) - инициализация пакета tasks
- `backend/main.py` (обновлен) - добавлена настройка APScheduler и запуск фоновой задачи
- `backend/requirements.txt` (обновлен) - добавлен APScheduler>=3.10.0

**Особенности:**
- Задача работает в фоновом режиме и не блокирует основной поток приложения
- Автоматическая остановка scheduler при завершении приложения
- Обработка ошибок с логированием и rollback транзакций
- Использование timezone-aware datetime для корректной работы с PostgreSQL

**Пример работы:**
1. Пользователь создает заказ со статусом "new" и `reserved_until = now + 30 минут`
2. Если заказ не оплачен в течение 30 минут, фоновая задача автоматически отменяет его
3. Статус заказа меняется на "cancelled"
4. Операция логируется для отслеживания

---

### [2025-12-14] - Удаление ограничений на размер загружаемых изображений

**Выполнено:**
- Убраны ограничения на размер загружаемых файлов
- Обновлена документация endpoints загрузки изображений
- Обновлено описание API: "торговая площадка" → "магазин"

**Файлы:**
- `backend/app/utils/file_upload.py` (обновлен) - убрана проверка размера файла и константа MAX_FILE_SIZE
- `backend/app/api/v1/uploads.py` (обновлен) - убраны упоминания о максимальном размере из документации
- `backend/main.py` (обновлен) - изменено описание API

**Изменения:**
- Функция `save_uploaded_file()` больше не принимает параметр `max_size`
- Убрана проверка размера файла при загрузке
- Теперь можно загружать изображения любого размера

---

### [2025-12-14] - Добавление обработки загрузки изображений для товаров и отзывов

**Выполнено:**
- Создана система загрузки и обработки изображений
- Добавлены endpoints для загрузки изображений товаров и отзывов
- Реализована валидация файлов (формат, MIME тип)
- Добавлена автоматическая оптимизация изображений (сжатие и изменение размера)
...
**Новые endpoints:**
- `POST /api/v1/uploads/products` - загрузка одного изображения для товара (требует админ)
- `POST /api/v1/uploads/products/multiple` - загрузка нескольких изображений для товара (требует админ, макс. 10)
- `POST /api/v1/uploads/reviews` - загрузка одного изображения для отзыва (требует JWT)
- `POST /api/v1/uploads/reviews/multiple` - загрузка нескольких изображений для отзыва (требует JWT, макс. 5)
- `DELETE /api/v1/uploads/{file_path}` - удаление загруженного файла (требует админ)

**Файлы:**
- `backend/app/utils/file_upload.py` (новый) - утилиты для работы с файлами
- `backend/app/utils/__init__.py` (новый) - инициализация пакета utils
- `backend/app/api/v1/uploads.py` (новый) - endpoints для загрузки файлов
- `backend/main.py` (обновлен) - добавлена раздача статических файлов и роутер uploads
- `backend/requirements.txt` (обновлен) - добавлен Pillow>=10.0.0
- `backend/.gitignore` (обновлен) - добавлена директория uploads
- `backend/uploads/.gitkeep` (новый) - сохранение структуры директории

**Особенности:**
- Поддерживаемые форматы: JPG, JPEG, PNG, GIF, WEBP
- Без ограничений на размер файла
- Автоматическая оптимизация: изменение размера до 1920x1920 и сжатие JPEG
- Безопасность: валидация MIME типов и расширений файлов
- Уникальные имена файлов: использование UUID для предотвращения конфликтов
- Статические файлы доступны по URL: `/uploads/products/...` и `/uploads/reviews/...`

**Использование:**
1. Загрузить изображение через endpoint `/api/v1/uploads/products` или `/api/v1/uploads/reviews`
2. Получить URL загруженного изображения в ответе
3. Использовать этот URL при создании/обновлении товара или отзыва через поля `image` или `images`

**Пример:**
```python
# 1. Загрузить изображение
POST /api/v1/uploads/products
Response: {"url": "/uploads/products/abc123.jpg"}

# 2. Создать товар с этим изображением
...
### [2025-12-14] - Захардкодирование данных подключения и полное тестирование

**Выполнено:**
- Захардкожены данные подключения к PostgreSQL в `app/database.py`
- Захардкожен SECRET_KEY в `app/auth.py`
- Захардкожены CORS origins в `main.py` (добавлен `https://109.73.202.83`)
- Захардкожен DATABASE_URL в `alembic/env.py`
- Убраны зависимости от `.env` файла и `python-dotenv` (код больше не использует `load_dotenv()`)
- Проверено подключение к реальной базе данных PostgreSQL
- Запущены все тесты: **51 тест прошли успешно** ✅
...
- CORS origins: `http://localhost:3000`, `http://localhost:3001`, `http://localhost:5173`, `https://109.73.202.83`

**Файлы:**
- `backend/app/database.py` (обновлен) - захардкожен DATABASE_URL
- `backend/app/auth.py` (обновлен) - захардкожен SECRET_KEY
- `backend/main.py` (обновлен) - захардкожены CORS origins
- `backend/alembic/env.py` (обновлен) - захардкожен DATABASE_URL
- `backend/test_connection.py` (новый) - скрипт для проверки подключения

**Результаты тестирования:**
- ✅ Все 51 тест прошли успешно
- ✅ Подключение к PostgreSQL работает корректно
- ✅ Все таблицы созданы в базе данных (users, products, cart_items, orders, order_items, order_comments, reviews)
- ⚠️ Есть предупреждения о deprecation (dict() -> model_dump(), datetime.utcnow() -> datetime.now(datetime.UTC))

**Примечание:** Данные захардкожены для упрощения развертывания. В продакшене рекомендуется использовать переменные окружения.
